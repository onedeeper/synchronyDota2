new_notrend <- function (x, B = 1000, test = c("t", "MK", "WAVK"), ar.method = "HVK", 
                         ar.order = NULL, BIC = TRUE, factor.length = c("user.defined", 
                                                                        "adaptive.selection"), Window = NULL, q = 3/4, j = c(8:11)) 
{
  ...
  if (identical(test, "WAVK")) {
    ...
    if (factor.length == "adaptive.selection" & length(kn) < 
        3) {
  ...
  if (!is.null(ar.order) & (NCOL(ar.order) > 1 | !is.numeric(ar.order) | 
                            NROW(ar.order) > 1)) {
  ...
  t <- c(1:n)/n
  pheta <- ARest(x, ar.order = ar.order, ar.method = ar.method, 
                 BIC = BIC)
  if (length(pheta) > 0) {
    names(pheta) <- paste(rep("phi_", length(pheta)), c(1:length(pheta)), 
                          sep = "")
    tmp <- stats::filter(x, pheta, sides = 1)
    Z <- x[(length(pheta) + 1):n] - tmp[length(pheta):(n - 
                                                         1)]
    for (i in 1:B) {
      e <- sample(Z, size = n, replace = TRUE)
      Y[, i] <- arima.sim(list(order = c(length(pheta), 
                                         0, 0), ar = pheta), n = n, innov = e)
    }
  }
  else {
    Z <- x
  ...
  if (test == "t") {
    ...
    sum.objs <- lapply(1:dim(Y)[2], function(i) summary(lm(Y[, 
                                                            i] ~ t)))
    boot.stat <- sapply(sum.objs, function(x) x$coefficients['t', 't value'])
    slope.ests <- sapply(sum.objs, function(x) x$coefficients['t', 'Estimate'])
  }
  if (test == "MK") {
    ...
    boot.stat <- sapply(1:dim(Y)[2], function(i) MannKendall(Y[, 
                                                               i])$tau)
  }
  if (test == "WAVK") {
    ...
    if (length(kn) < 3) {
      kn_opt <- kn[1]
      boot.stat <- sapply(1:dim(Y)[2], function(j) WAVK(Y[, 
                                                          j], kn_opt)$Tns)
    }
    else {
      s <- array(data = NA, c(length(kn), B))
      for (i in 1:length(kn)) {
        s[i, ] <- sapply(1:dim(Y)[2], function(j) WAVK(Y[, 
                                                         j], kn[i])$Tns)
      }
      s <- t(apply(s, 1, sort))
      distance <- sapply(1:(length(kn) - 1), function(x) dist(s[x:(x + 
                                                                     1), ]))
  ...
  if (test == "WAVK") {
    ...
    structure(list(method = METHOD, data.name = DNAME, statistic = STATISTIC, 
                   p.value = P.VALUE, alternative = ALTERNATIVE, estimate = ESTIMATE, slopes = slope.ests), 
              class = "htest")
  }
}
